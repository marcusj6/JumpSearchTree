...
static void BTreeDelete1(BTree *tree, const Key *key);
static void BTreeDelete2Recursion(BTree *tree, BTreeNode *current, const Key *key);
static void BTreeDelete2(BTree **tree, const Key *key);
...
static void BTreeDelete1(BTree *tree, const Key *key) {
	//Will be optimized version of Jan Jannink's algorithm
	//TODO: IMPLEMENT
}
...
static void BTreeDelete2Recursion(BTree *tree, BTreeNode *current, const Key *key) {
	...
	if (current->values != NULL) { //Leaf
		...
		BTreeDelete2Recursion(tree, current->children[i], key);
...
static void BTreeDelete2(BTree **tree, const Key *key) {
	//Will use no rebalance with no rebuild
	BTreeDelete2Recursion((*tree), (*tree)->root, key);
}
